...
void delete_search_value(Node*& head, Node*& tail, Date date)
{
	...
	while (current->next != nullptr)
	{
		while (current->previos == nullptr && 
			(current->ticket.date.year < date.year || 
			current->ticket.date.year == date.year &&
			current->ticket.date.month < date.month ||
			current->ticket.date.year == date.year &&
			current->ticket.date.month == date.month &&
			current->ticket.date.day < date.day))
		{
			current = head = delete_head(current, tail);
		}
		if (tail->ticket.date.year < date.year ||
			tail->ticket.date.year == date.year &&
			tail->ticket.date.month < date.month ||
			tail->ticket.date.year == date.year &&
			tail->ticket.date.month == date.month &&
			tail->ticket.date.day < date.day)
		{
			delete_tail(tail, current);
		}

		else if (current->previos == nullptr &&
			     (current->ticket.date.year < date.year ||
				  current->ticket.date.year == date.year &&
				  current->ticket.date.month < date.month ||
				  current->ticket.date.year == date.year &&
